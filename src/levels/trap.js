...
export default class Trap extends Environment {
  constructor(options = {}) {
    super(options);
    const {
      y = 0,
  ...
  cerateTrap() {
    ...
    for (let i = 0; i < actualCount; i++) {
      ...
      this.dummy.position.set(x, this.y + (this.cellSize / 5 + 0.1) / 2, z);
      this.dummy.updateMatrix();
      this.instanced.setMatrixAt(i, this.dummy.matrix);

      this.setCell(cell, { type: 'trap' });
      trapCells.push(cell);
    }

    this.instanced.instanceMatrix.needsUpdate = true;
    this.setMesh(this.instanced, trapCells);
